...
def evaluation_onebyone(args):
    ...
    for subdir in subdirs:
        ...
        files=os.listdir(args.path+'/'+subdir)
        evalstatic.total=len(files)
        for file in files:
            imgpath=args.path+'/'+subdir+'/'+file
...
def evaluation_batch(args):
    ...
    for subdir in subdirs:
        ...
        files=os.listdir(args.path+'/'+subdir)
        evalstatic.total=len(files)
        inputs=[caffe.io.load_image(args.path+'/'+subdir+'/'+file) for file in files]
        try:
            predictions = classifier.predict(inputs,oversample=False)
        except Exception as e:
            print(e)
        for i in range(len(files)):
            ...
            if subdir!=label:
                ...
                if not os.path.exists(args.errordir+'/'+subdir):
                    os.mkdir(args.errordir+'/'+subdir)
                imgpath=args.path+"/"+subdir+"/"+files[i]
...
def evaluation_dir(args):
    clearlasterrors(args)
    if args.evaluationonebyone:
        evaluation_onebyone(args)
    else:
        evaluation_batch(args)

def evaluation_image(args):
    labels=[w.split()[1] for w in open(args.labelfile).readlines()]
    classifier=getclassifier(args)
    inputs=[caffe.io.load_image(args.path)]
    predictions = classifier.predict(inputs,oversample=False)
    p=predictions[0,:].argmax()
    label=labels[p]
...
def evaluation(args):
    if os.path.isdir(args.path):
        ...
        evaluation_image(args)

def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--iter",default=10000,help="caffemodel iter to evaluation")
    parser.add_argument("--path",default="../data",help="data dir to evaluation")
    parser.add_argument("--image_dims",default=[227,227],help="image_dims")
    ...
    parser.add_argument("--evaluationonebyone",default=False,help="log txt")
